...
export class Application<StateT = DefaultState, ContextT = DefaultContext> extends Koa implements AsyncEmitter {
  ...
  createCli() {
    ...
    command.exitOverride((err) => {
      if (err.code === 'commander.help') {
        process.exit(command.args.length > 0 ? 1 : 0);
      }
      if (err.code === 'commander.executeSubCommandAsync' || err.code === 'commander.helpDisplayed') {
        process.exit(0);
      }
    });
  ...
  protected init() {
    ...
    this.command('exec [values...]')
      .action(async (values) => {
        console.log(values);
        // FIXME: find solution to wait for plugins loaded
        await this.wait(5000);
        try {
          await this.runAsCLI([process.argv[0], process.argv[1], ...values])
        } catch (error) {
          console.log(error);
        }
      });
